...
namespace Netcode.Rollback.Sessions
{
        ...
        where TInput : IInput<TInput>
    {
        ...
        public SpectatorSession(
            ...
            UdpProtocol<TInput, TAddress> host,
            uint maxFramesBehind,
            uint catchupSpeed
        )
        {
        ...
        public List<RollbackRequest<TState, TInput>> AdvanceFrame()
        {
            ...
            {
                throw new InvalidOperationException("not synchronized yet");
            }
            ...
            for (int i = 0; i < framesToAdvance; i++)
            {
                ...
                requests.Add(
                    RollbackRequest<TState, TInput>.From(
                        new RollbackRequest<TState, TInput>.AdvanceFrame { Inputs = syncedInputs }
                    )
                );
        ...
        public void PollRemoteClients()
        {
            ...
            {
                if (_host.IsHandlingMessage(from))
                {
                    _host.HandleMessage(msg);
                }
            }

            Deque<Event<TInput>> events = new Deque<Event<TInput>>();
            TAddress addr = _host.PeerAddr;
            foreach (Event<TInput> ev in _host.Poll(_hostConnectStatus))
            {
                events.PushBack(ev);
            }
            while (events.Count > 0)
            {
                HandleEvent(events.PopFront(), addr);
            }
        ...
        private (TInput input, InputStatus status)[] InputsAtFrame(Frame frameToGrab)
        {
            ...
            {
                if (_hostConnectStatus[i].Disconnected && _hostConnectStatus[i].LastFrame < frameToGrab)
                {
                    res[i] = (inputs[i].Input, InputStatus.Disconnected);
                }
                else
                {
                    res[i] = (inputs[i].Input, InputStatus.Confirmed);
                }
            }
            return res;
        }
        ...
        private void HandleEvent(Event<TInput> ev, TAddress addr)
        {
            switch (ev.Kind)
            {
                ...
                case EventKind.Input:
                    ...
                    _inputs[input.Data.Frame.No % SpectatorConstants.SPECTATOR_BUFFER_SIZE][input.Player.Id] =
                        input.Data;
                    ...
                    for (int i = 0; i < _numPlayers; i++)
                    {
                        _hostConnectStatus[i] = _host.PeerConnectStatus(new PlayerHandle(i));
                    }
                    break;
            }
