    ...
    public partial class GameState : IState<GameState>
    {
        ...
        private void ApplySuperInputLock(SimOptions options, Span<GameInput> remapInputs)
        {
            for (int i = 0; i < Fighters.Length; i++)
            {
                ...
                HitboxData hd = options.Players[i ^ 1].GetHitboxData(s);
        ...
        public void Advance(SimOptions options, (GameInput input, InputStatus status)[] inputs)
        {
            ...
            for (int i = 0; i < Fighters.Length; i++)
            {
                Fighters[i].ApplyAerialCancel(SimFrame, options, options.Players[i]);
            }
        ...
        private void DoCollisionStep(SimOptions options)
        {
            ...
            for (int i = 0; i < Fighters.Length; i++)
            {
                Fighters[i].AddBoxes(SimFrame, options.Players[i], PhysicsCtx.Physics, i);
            }
        ...
        )
        {
            ...
            if (attacker.Data.Kind == HitboxKind.Grabbox)
            {
                ...
                Fighters[attacker.Owner].ProcessHit(SimFrame, attacker.Data, options.Players[attacker.Owner]);
            ...
            if (outcome.Kind == HitKind.Hit)
            {
                ...
                Fighters[attacker.Owner]
                    .ProcessHit(SimFrame, attacker.Data, options.Players[attacker.Owner]);
            }
