...
namespace Hypermania.Game
{
    ...
    public partial struct FighterState
    {
            ...
            if (frame >= StateEnd)
            {
                ...
                if (State == CharacterState.HeavyKnockdown)
                {
                    SetState(
                        ...
                        frame,
                        frame + options.Players[Index].GetHitboxData(CharacterState.GetUp).TotalTicks
                    );
                    return;
                }
        ...
        {
            PlayerSimOptions player = options.Players[Index];
            CharacterStats config = player.Character;
            ...
            bool gatlingPreJumpAllowed =
                ...
                && IsGatlingCancelAllowed(CharacterState.PreJump, frame, player);
        ...
        private void TriggerPreJump(Frame frame, SimOptions options, bool isRhythmCancel, sfloat runMult)
        {
            ...
            SetState(CharacterState.PreJump, frame, frame + options.Players[Index].GetHitboxData(CharacterState.PreJump).TotalTicks);
        }
        ...
        )
        {
            ...
            )
            {
                ...
                SetState(
                    ...
                    simFrame,
                    simFrame + player.GetHitboxData(CharacterState.Burst).TotalTicks
                );
                return;
            }
            ...
            // Followup attacks:
            HitboxData curData = player.GetHitboxData(State);
            ...
            )
            {
                ...
                if (isRhythmCancel)
                {
                    ...
                    startFrame -= player.GetHitboxData(curData.Followup).StartupTicks;
                }
                AttackConnected = false;
                SetState(
                    curData.Followup,
                    startFrame,
                    startFrame + player.GetHitboxData(curData.Followup).TotalTicks,
                    true
                );
                return;
            }
            ...
            foreach (((var loc, var input), var state) in _attackDictionary)
            {
                ...
                if (!canActNormally && !IsGatlingCancelAllowed(state, simFrame, player))
                {
                    continue;
                }
                ...
                Frame startFrame = simFrame;
                if (isRhythmCancel && player.GetHitboxData(state).IsValidAttack(frames))
                {
                    // Beat-snap: back-date StateStart by the attack's startup
                    // so the active frame lands on simFrame itself (= the
                    // note's dispatch frame, since ManiaState withholds the
                ...
                AttackConnected = false;
                SetState(state, startFrame, startFrame + player.GetHitboxData(state).TotalTicks, true);
                return;
            }
        ...
        private bool IsGatlingCancelAllowed(CharacterState to, Frame simFrame, PlayerSimOptions player)
        {
            if (!AttackConnected)
                return false;
            if (!player.Character.HasGatling(State, to))
                return false;

            HitboxData fromData = player.GetHitboxData(State);
            ...
            HitboxData toData = player.GetHitboxData(to);
        ...
        public void UpdatePosition(Frame frame, SimOptions options, ref SVector2 otherFighterPos)
        {
            ...
            FrameData curData = options.Players[Index].GetFrameData(State, frame - StateStart);
            if (curData.Floating)
            {
            ...
            HitboxData moveData = options.Players[Index].GetHitboxData(State);
            if (moveData != null && moveData.ApplyRootMotion)
            {
                int rmTick = frame - StateStart;
                SVector2 prevOffset =
                    rmTick > 0
                        ? options.Players[Index].GetFrameData(State, rmTick - 1).RootMotionOffset
                        : SVector2.zero;
        ...
        public void ApplyAerialCancel(Frame frame, SimOptions options, PlayerSimOptions player)
        {
            if (!OnGround(options))
            {
                return;
            ...
            if (State.IsAerialAttack())
            {
                ...
                SetState(
                    ...
                    frame,
                    frame + player.GetHitboxData(CharacterState.Landing).TotalTicks
                );
                return;
            }
            ...
            if (State == CharacterState.HeavyKnockdown && PendingKnockdown == KnockdownKind.Light)
            {
                ...
                SetState(
                    ...
                    frame,
                    frame + player.GetHitboxData(CharacterState.SoftKnockdown).TotalTicks,
                    true
                );
                return;
            }
        ...
        public void AddBoxes(Frame frame, PlayerSimOptions player, Physics<BoxProps> physics, int handle)
        {
            int tick = frame - StateStart;
            HitboxData hitboxData = player.GetHitboxData(State);
            FrameData frameData = player.GetFrameData(State, tick);
        ...
        public void ProcessHit(Frame frame, BoxProps props, PlayerSimOptions player)
        {
            if (props.HasTransition)
            {
                ...
                if (props.OnHitTransition == CharacterState.Throw)
                {
                    ...
                    EnqueueHitTransition(
                        ...
                        nextStart,
                        nextStart + player.GetHitboxData(CharacterState.Throw).TotalTicks,
                        true
                    );
                    return;
                }
                EnqueueHitTransition(
                    ...
                    nextStart,
                    nextStart + player.GetHitboxData(props.OnHitTransition).TotalTicks,
                    true
                );
            }
        }
